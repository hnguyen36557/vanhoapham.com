...
class WelcomeController extends Controller {
	...
	*/
	...
	public function index()
	{
		...
		$products = DB::table('products')->select('id','name','image','price','alias')->orderBy('id','ASC')->skip(0)->take(4)->get();
				return view('user.pages.home',compact('product','products'));
	}
	public function loaisanpham($id) {
		$product_cate = DB::table('products')->select('id','name','image','price','alias','cate_id')->where('cate_id',$id)->paginate(5);
	...
	public function post_lienhe(Request $request) {
		...
		Mail::send('emails.blanks',$data,function($msg) {
			$msg->from('thanhtamoop@gmail.com','Hiếu Nguyễn');
			$msg->to('hnguyen36557@gmail.com','Hoài Lê')->subject('chào cưng');
		});
		echo "<script>
			alert('Cảm ơn đã góp ý. Chúng tôi sẽ liên hệ trong thời gian sớm nhất');
			window.location = '".url('/')."'
		</script>";
	}
